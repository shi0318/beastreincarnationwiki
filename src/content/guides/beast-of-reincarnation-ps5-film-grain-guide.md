...
1. Check that the game is fully updated.
2. Look through the available display and accessibility options after the update.
...
4. If the effects remain uncomfortable, report the platform, display, game version, scene, and the requested toggle in the official discussion.
5. Recheck the developer thread after a patch instead of assuming a planned option has already shipped.
...
The official v1.0.8 patch notes add camera choices and other PC/gameplay changes, but the checked note does not provide a shipped PS5 film-grain toggle or a date for the planned post-processing options. If a future patch adds the control, record the exact version and menu path here rather than silently replacing the current status.

Until then, the honest answer is: **PS5 players can request the toggle and watch the official thread, but there is no confirmed in-game switch to disable film grain and chromatic aberration today.**
