...
The original Steam post describes a quest that asks the player to defeat headless Golems in Fuji Frontier. The player sees three Golems, but none is the required headless variant, and killing them repeatedly does not complete the quest. The same post says a target from a second quest does not spawn either.

Replies add several related reports:

- a PS5 player said the same issue appeared after the latest update;
- an Xbox Game Pass player reported the bug too;
- another player could not spawn a wolf-like enemy at a shrine;
- a separate player reported an invisible sword after loading into the zone;
- the developer replied that the report had been sent to the team.

These reports show a useful pattern, but they do not prove that every missing enemy, invisible weapon, or empty-looking map has one cause. Keep those symptoms separate in a support report.

## Low-risk checks before reporting
...
5. **Do not farm the visible Golems as a substitute for the target.** If the required variant is missing, repeated kills may only waste time and can make the report harder to describe.
